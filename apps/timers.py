...
def before_the_holidays_computing(): #считаеться сколько осталось до всех каникул времени
    ...
    autumn_holidays = datetime.datetime(year, config.beginning_autumn_holidays_month, config.beginning_autumn_holidays_day)
    summer_holidays = datetime.datetime(year, config.beginning_summer_holidays_month,config.beginning_summer_holidays_day)
    spring_holidays = datetime.datetime(year,config.beginning_spring_holidays_month,config.beginning_spring_holidays_day)
    winter_holidays = datetime.datetime(year,config.beginning_winter_holidays_month,config.beginning_winter_holidays_day)
...
def before_the_holidays_text(): #полученых значения конвертируются в текст
    remained_until_summer, remained_until_autumn, remained_until_spring, remained_until_winter = before_the_holidays_computing()

    if remained_until_summer < remained_until_autumn and remained_until_summer < remained_until_spring and remained_until_summer < remained_until_winter:
        mm, ss = divmod(remained_until_summer.seconds, 60)
        hh, mm = divmod(mm, 60)
        remained_until_summer_text = ('До летних каникул осталось: {} дн. {} ч. {} мин. {} сек.⏳'.format(remained_until_summer.days, hh, mm, ss))
        return remained_until_summer_text
        
    if remained_until_autumn < remained_until_summer and remained_until_autumn < remained_until_spring and remained_until_autumn < remained_until_winter:
        mm, ss = divmod(remained_until_autumn.seconds, 60)
        hh, mm = divmod(mm, 60)
        remained_until_autumn_text = ('До осенних каникул осталось: {} дн. {} ч. {} мин. {} сек.⏳'.format(remained_until_autumn.days, hh, mm, ss))
        return remained_until_autumn_text

    if remained_until_spring < remained_until_autumn and remained_until_spring < remained_until_summer and remained_until_spring < remained_until_winter:
        mm, ss = divmod(remained_until_spring.seconds, 60)
        hh, mm = divmod(mm, 60)
        remained_until_spring_text = ('До весенних каникул осталось: {} дн. {} ч. {} мин. {} сек.⏳'.format(remained_until_spring.days, hh, mm, ss))
        return remained_until_spring_text

    if remained_until_winter < remained_until_autumn and remained_until_winter < remained_until_spring and remained_until_winter < remained_until_summer:
        mm, ss = divmod(remained_until_winter.seconds, 60)
        hh, mm = divmod(mm, 60)
        remained_until_winter_text = ('До зимних каникул осталось: {} дн. {} ч. {} мин. {} сек.⏳'.format(remained_until_winter.days, hh, mm, ss))
...
def identification_holidays(): # проверка сейчас ли каникулы
    ...
    elif datetime.datetime(year,config.beginning_winter_holidays_month,config.beginning_winter_holidays_day) <= datetime.datetime(year,month,day) and datetime.datetime(year,month,day) <= datetime.datetime(year,config.end_winter_holidays_month,config.end_winter_holidays_day):
        remained_until_holidays= "winter"
        end_holidays_text = until_the_end_o_the_holidays(holidays)
        return end_holidays_text
           
    elif datetime.datetime(year,config.beginning_spring_holidays_month,config.beginning_spring_holidays_day) <= datetime.datetime(year,month,day) and datetime.datetime(year,month,day) <= datetime.datetime(year,config.end_spring_holidays_month,config.end_spring_holidays_day):
        remained_until_holidays= "spring"
        end_holidays_text = until_the_end_o_the_holidays(holidays)
        return end_holidays_text
    
    elif datetime.datetime(year,config.beginning_autumn_holidays_month,config.beginning_autumn_holidays_day) <= datetime.datetime(year,month,day) and datetime.datetime(year,month,day) <= datetime.datetime(year,config.end_autumn_holidays_month,config.end_autumn_holidays_day):
        remained_until_holidays= "autumn"
        end_holidays_text = until_the_end_o_the_holidays(holidays)
        return end_holidays_text
    else:
...
def until_the_end_o_the_holidays(holidays): # сколько осталось до конца каникул
    ...
    if holidays == "summer":
        ...
        hh, mm = divmod(mm, 60)
        end_holidays_text = ('Сейчас весенние каникулы🍃 \n\nДо их конца осталось:\n{} дн. {} ч. {} мин. {} сек.⏳'.format(end_spring.days, hh, mm, ss))
        return end_holidays_text
        ...
        hh, mm = divmod(mm, 60)
        end_holidays_text = ('Сейчас осение каникулы🍂 \n\nДо их конца осталось:\n{} дн. {} ч. {} мин. {} сек.⏳'.format(end_autumn.days, hh, mm, ss))
        return end_holidays_text
        ...
        hh, mm = divmod(mm, 60)
        end_holidays_text = ('Сейчас зимние каникулы❄ \n\nДо их конца осталось:\n{} дн. {} ч. {} мин. {} сек.⏳'.format(end_winter.days, hh, mm, ss))
        return end_holidays_text